...
def venues():
  ...
  for local in db.session.query(distinct(Venue.city),Venue.state).group_by(Venue.state,Venue.city).all():
    ven_info = {}
    ven_list = []
    ven_local['city'] = local[0]
    ven_local['state'] = local[1]
    #Add all venues associated with each city/state
    for ven in db.session.query(Venue.id,Venue.name).filter(Venue.city==local[0],Venue.state==local[1]).all():
...
def search_venues():
  ...
  #Query with raw SQL
  venue_query = db.session.execute("SELECT \"Venue\".id,\"Venue\".name FROM \"Venue\" WHERE \"Venue\".name ILIKE \'%{}%\'".format(search))
  for venue in venue_query:
    data.append(venue)

  #Store searched venue data inside respond
  response1["count"] = len(data)
  response1["data"] = data
...
def search_artists():
  ...
  artist_query = db.session.execute("SELECT \"Artist\".id,\"Artist\".name FROM \"Artist\" WHERE \"Artist\".name ILIKE \'%{}%\'".format(search))
  for artist in artist_query:
    data.append(artist)
